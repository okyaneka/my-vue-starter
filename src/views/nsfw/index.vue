...
function generateImage() {
  ...
  for (let index = 0; index < amount.value; index++) {
    _images.push(`${baseUrl.value}/${index + 1}.jpg`)
  }
...
<template>
  <div class="p-4 grid grid-cols-4 gap-4 border rounded mb-4 border-solid border-gray-400">
    <div
      class="p-4 h-32 border rounded border-solid border-gray-400"
      :class="{'bg-blue-200': url == baseUrl}"
      v-for="url in urls"
      @click="salin(url)"
    >
      <img class="w-full h-full object-cover" :src="`${url}/p=160x200/1.jpg`" />
    </div>
  </div>
  ...
  <div class="grid grid-cols-3">
    ...
    >
      ...
      <a :href="src" target="_blank">
        ...
        <img class="m-0 w-full h-full object-contain" :src="src" />
      </a>
